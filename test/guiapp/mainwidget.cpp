...
void MainWidget::procPelcoPButtonClick()
{
    if (this->initPelcoPWidget() && this->serial && this->serial->isOpen()) {
        qDebug() << "PelcoP" << endl;
        this->pelcoPWidget->setTitle(this->serial->deviceName());
        this->pelcoPWidget->show();
    }
}
...
void MainWidget::initMainWidgetCloseState()
{
    ...
    ui->pelcoPButton->setEnabled(false);
...
void MainWidget::initMainWidgetOpenState()
{
    ...
    ui->pelcoPButton->setEnabled(true);
...
bool MainWidget::initPelcoPWidget()
{
    if (!this->pelcoPWidget) {
        this->pelcoPWidget = new PelcoPWidget();
        if (!this->pelcoPWidget)
            return false;

        connect(this->pelcoPWidget, SIGNAL(sendSerialData(QByteArray)),
                this, SLOT(procSerialDataTransfer(QByteArray)));
    }
    return true;
}
...
void MainWidget::initButtonConnections()
{
    ...
    connect(ui->pelcoPButton, SIGNAL(clicked()), this, SLOT(procPelcoPButtonClick()));
